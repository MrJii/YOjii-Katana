...
function redirect_canonical( $requested_url = null, $do_redirect = true ) {
	...
	if ( is_404() ) {
		...
		if ( 'wp-register.php' == basename( $redirect['path'] ) ) {
			if ( is_multisite() ) {
				/** This filter is documented in YOjii-Katana-login.php */
				$redirect_url = apply_filters( 'wp_signup_location', network_site_url( 'YOjii-Katana-signup.php' ) );
			} else {
				$redirect_url = site_url( 'YOjii-Katana-login.php?action=register' );
			}
...
function wp_redirect_admin_locations() {
	...
	$logins = array(
		home_url( 'YOjii-Katana-login.php', 'relative' ),
		home_url( 'login', 'relative' ),
		site_url( 'login', 'relative' ),
	);
	if ( in_array( untrailingslashit( $_SERVER['REQUEST_URI'] ), $logins ) ) {
		wp_redirect( site_url( 'YOjii-Katana-login.php', 'login' ) );
		exit;
	}
}
