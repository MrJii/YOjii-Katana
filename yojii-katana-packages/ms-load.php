...
function ms_not_installed() {
	...
	$title = __( 'SORRY! We cannot define database connection... :(' );
	$msg  = '<h1>' . $title . '</h1>';
	if ( ! is_admin() ) {
		die( $msg );
	}
	...
	if ( ! $wpdb->get_var( $query ) ) {
		$msg .= '<p>' . sprintf( __( '<strong>Database tables are missing.</strong> This means that MySQL is not running, YOjii-Katana was not installed properly, or someone deleted <code>%s</code>. You really should look at your database now.' ), $wpdb->site ) . '</p>';
	} else {
