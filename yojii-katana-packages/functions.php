...
function wp_upload_dir( $time = null ) {
	...
	if ( empty( $upload_path ) || 'yojii-katana-site/uploads' == $upload_path ) {
	...
	if ( !$url = get_option( 'upload_url_path' ) ) {
		if ( empty($upload_path) || ( 'yojii-katana-site/uploads' == $upload_path ) || ( $upload_path == $dir ) )
	...
	if ( is_multisite() && ! ( is_main_network() && is_main_site() && defined( 'MULTISITE' ) ) ) {
		...
		if ( ! get_site_option( 'ms_files_rewriting' ) ) {
			 ...
			 * had yojii-katana-site/uploads for the main site.)
			 */
...
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
	<title><?php _e( 'MySQL Database Error' ); ?></title>

</head>
<body>
	<h1><?php _e( 'SORRY! We cannot find database connection... :(' ); ?></h1>
</body>
</html>
<?php
	die();
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		if ( strpos( $_SERVER['REQUEST_URI'], 'wp-admin' ) !== false || strpos( $_SERVER['REQUEST_URI'], 'YOjii-Katana-login.php' ) !== false ) {
			$path = preg_replace( '#/(wp-admin/.*|YOjii-Katana-login.php)#i', '', $_SERVER['REQUEST_URI'] );
...
/**
 * Output the HTML that shows the YOjii-Katana-login dialog when the user is no longer logged in.
 *
 * @since 3.6.0
 */
