...
function wp_dashboard_setup() {
	...
	// QuickPress Widget
	if ( is_blog_admin() && current_user_can( 'edit_posts' ) ) {
		$quick_draft_title = sprintf( '<span class="hide-if-no-js">%1$s</span> <span class="hide-if-js">%2$s</span>', __( 'Temporary Folder' ), __( 'Stored Folders' ) );
		wp_add_dashboard_widget( 'dashboard_quick_press', $quick_draft_title, 'wp_dashboard_quick_press' );
	}

	// WordPress News
	wp_add_dashboard_widget( 'dashboard_primary', __( 'YOjii-Katana News' ), 'wp_dashboard_primary' );
...
?>
	...
	<form name="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press" class="initial-form hide-if-no-js">
		...
		<p class="submit">
			...
			<?php submit_button( __( 'Save Folder' ), 'primary', 'save', false, array( 'id' => 'save-post' ) ); ?>
			<br class="clear" />
		</p>
...
function wp_dashboard_cached_rss_widget( $widget_id, $callback, $check_urls = array() ) {
	$loading = '<p class="widget-loading hide-if-no-js">' . __( 'YOjii-Katana\'s Loading&#8230;' ) . '</p><p class="hide-if-js">' . __( 'This widget requires JavaScript.' ) . '</p>';
...
function wp_dashboard_primary() {
	$feeds = array(
		'news' => array(
			 ...
			 */
			'link' => apply_filters( 'dashboard_primary_link', __( 'http://jiiyo.wordpress.com/news/' ) ),
			 ...
			 */
			'url' => apply_filters( 'dashboard_primary_feed', __( 'http://jiiyo.wordpress.com/news/feed/' ) ),
			 ...
			 */
			'title'        => apply_filters( 'dashboard_primary_title', __( 'YOjii-Katana Blog' ) ),
			'items'        => 1,
			'show_summary' => 1,
			'show_author'  => 0,
			'show_date'    => 1,
		...
		'planet' => array(
			 ...
			 */
			'link' => apply_filters( 'dashboard_secondary_link', __( 'http://jiiyo.wordpress.com/' ) ),

			/**
			 * Filter the secondary feed URL for the 'YOjii-Katana News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $url The widget's secondary feed URL.
			 */
			'url' => apply_filters( 'dashboard_secondary_feed', __( 'http://jiiyo.wordpress.com/feed/' ) ),

			/**
			 * Filter the secondary link title for the 'YOjii-Katana News' dashboard widget.
			 *
			 * @since 2.3.0
			 *
			 * @param string $title Title attribute for the widget's secondary link.
			 */
			'title'        => apply_filters( 'dashboard_secondary_title', __( 'Other YOjii-Katana News' ) ),
			'items'        => 3,
			'show_summary' => 0,
			'show_author'  => 0,
			'show_date'    => 0,
...
function wp_dashboard_plugins_output( $rss, $args = array() ) {
	...
	foreach ( array( $popular ) as $feed ) {
		...
		echo "<li class='dashboard-news-plugin'><span>" . __( 'Our Latest Module' ) . ":</span> <a href='$link' class='dashboard-news-plugin-link'>$title</a>&nbsp;<span>(<a href='$ilink' class='thickbox' title='$title'>" . __( 'Install' ) . "</a>)</span></li>";
...
function wp_check_browser_version() {
	...
	if ( false === ($response = get_site_transient('browser_' . $key) ) ) {
		...
		$options = array(
			...
			'user-agent'	=> 'YOjii-Katana/' . $wp_version . '; ' . home_url()
		);

		$response = wp_remote_post( 'http://github.com/MrJii/YOjii-Katana', $options );
...
function wp_welcome_panel() {
	...
	<h3><?php _e( 'Welcome to YOjii-Katana!' ); ?></h3>
	<p class="about-description"><?php _e( 'We&#8217;ve assembled some links to get you started:' ); ?></p>
	<div class="welcome-panel-column-container">
	<div class="welcome-panel-column">
		<?php if ( current_user_can( 'customize' ) ): ?>
			<h4><?php _e( 'Get Started' ); ?></h4>
			<a class="button button-primary button-hero load-customize hide-if-no-customize" href="<?php echo wp_customize_url(); ?>"><?php _e( 'Personalize Your Site' ); ?></a>
		<?php endif; ?>
		<a class="button button-primary button-hero hide-if-customize" href="<?php echo admin_url( 'themes.php' ); ?>"><?php _e( 'Personalize Your Site' ); ?></a>
