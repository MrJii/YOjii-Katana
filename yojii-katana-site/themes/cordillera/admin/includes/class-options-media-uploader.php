...
class Options_Framework_Media_Uploader {
	...
	static function optionsframework_uploader( $_id, $_value, $_desc = '', $_name = '' ) {
		...
		$output .= '<input id="' . $id . '" class="upload' . $class . '" type="text" name="'.$name.'" value="' . $value . '" placeholder="' . __('No file chosen', 'cordillera') .'" />' . "\n";
		if ( function_exists( 'wp_enqueue_media' ) ) {
			if ( ( $value == '' ) ) {
				$output .= '<input id="upload-' . $id . '" class="upload-button button" type="button" value="' . __( 'Upload', 'cordillera' ) . '" />' . "\n";
			} else {
				$output .= '<input id="remove-' . $id . '" class="remove-file button" type="button" value="' . __( 'Remove', 'cordillera' ) . '" />' . "\n";
			}
		} else {
			$output .= '<p><i>' . __( 'Upgrade your version of WordPress for full media support.', 'cordillera' ) . '</i></p>';
		}
	...
	function optionsframework_media_scripts( $hook ) {
		...
		wp_localize_script( 'of-media-uploader', 'optionsframework_l10n', array(
			'upload' => __( 'Upload', 'cordillera' ),
			'remove' => __( 'Remove', 'cordillera' )
		) );
	}
}