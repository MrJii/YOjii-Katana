Cordillera Theme
================

Cordillera is a business theme which you can use to build homepage for individual or company with flat design. With enriched options in section settings, you can not only add background color, background image for each section, but also set the section as full-width or not, divide the section into several columns, etc. We have predefined 10 frequently-used widgets which you can use in any section. The theme also has a full screen banner, four types of layout for blog&page, one-click for changeing main color and much more. Since it is based on Bootstrap and coded with HTML5 and CSS3 language, the theme is fully responsive in desktops and mobile devices.

## Copyrights for Resources used in this theme.

The theme itself is nothing but 100% GPL v2(or later).


Icons 
  ==image icons
 * License: The icons are free for personal use and also free for commercial use.
 * All icons are under GPL v2 have been created by our team.
 * Copyright: Mageewp.com, http://www.mageewp.com

  ==Font Awesome icons
 * License: License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
 * Copyright: by @davegandy - http://fontawesome.io - @fontawesome
--------------------------------------------------------------------------------------------

Images
  ==banner.jpg
 * Copyright: picjumbo.com

--------------------------------------------------------------------------------------------

Fonts 
 ==Font Awesome 4.3.0
 * License: License - http://fontawesome.io/license (Font: SIL OFL 1.1, CSS: MIT License)
 * Copyright: by @davegandy - http://fontawesome.io - @fontawesome

--------------------------------------------------------------------------------------------

Administration Panel 
 * License: For the Administration Panel, we have used "Options Framework", which is under GPL v2 license. 
 * Copyright: Devin Price, http://wptheming.com/options-framework-plugin/ 

--------------------------------------------------------------------------------------------

JS files

 ==bootstrap.js
 * Bootstrap v3.0.3 (http://getbootstrap.com)
 * Copyright 2013 Twitter, Inc.
 * Licensed under http://www.apache.org/licenses/LICENSE-2.0

 ==cordillera.js
 * License: GPLv2
 * Copyright: MageeWp, http://www.mageewp.com

 ==html5.js
 * HTML5 Shiv v3.7.0 | @afarkas @jdalton @jon_neal @rem | MIT/GPL2 Licensed

 == respond.min.js
 *! respond.min.js v1.4.2: min/max-width media query polyfill
 * Copyright 2014 Scott Jehl
 * Licensed under MIT
 * http://j.mp/respondjs */

 == modernizr.custom.js
 * Licensed: MIT & BSD
 * Copyright: modernizr.com

 == jquery.tubular.1.0.js
 * Licensed: licensed under the MIT License
 * Copyright: by Sean McCambridge
 * http://www.seanmccambridge.com/tubular


--------------------------------------------------------------------------------------------

PHP scripts
 == breadcrumb-trail.php
 == breadcrumbs.php
 * @author Justin Tadlock <justin@justintadlock.com>
 * @copyright Copyright (c) 2008 - 2013, Justin Tadlock
 * @link http://themehybrid.com/plugins/breadcrumb-trail
 * @license http://www.gnu.org/licenses/old-licenses/gpl-2.0.html


--------------------------------------------------------------------------------------------

	For any help you can mail me at support[at]mageewp.com


== CHANGE LOG == 

= Version 1.0.7 =
* Added YouTube Video Background Banner
* Fixed Home page widget issues

= Version 1.0.2 =
* Fixed Global Color issue
* Fixed custom css issue: replace > with &gt; in textarea 
* Added title-tag support
