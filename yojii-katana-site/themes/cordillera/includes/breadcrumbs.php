...
* @return void
*/
function cordillera_breadcrumb_trail( $args = array() ) {
if ( function_exists( 'is_bbpress' ) && is_bbpress() )
$breadcrumb = new Cordillera_bbPress_Breadcrumb_Trail( $args );
else
$breadcrumb = new Cordillera_Breadcrumb_Trail( $args );
...
* @since 0.6.0
*/
class Cordillera_Breadcrumb_Trail {
/**
...
$labels = array(
'browse' => __( 'Browse:', 'cordillera' ),
'home' => __( 'Home', 'cordillera' ),
'error_404' => __( '404 Not Found', 'cordillera' ),
'archives' => __( 'Archives', 'cordillera' ),
/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
'search' => __( 'Search results for &#8220;%s&#8221;', 'cordillera' ),
/* Translators: %s is the page number. */
'paged' => __( 'Page %s', 'cordillera' ),
/* Translators: Minute archive title. %s is the minute time format. */
'archive_minute' => __( 'Minute %s', 'cordillera' ),
/* Translators: Weekly archive title. %s is the week date format. */
'archive_week' => __( 'Week %s', 'cordillera' ),
...
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'cordillera' ) ) );
}
/**
...
/* Add the minute item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'cordillera' ) ) );
}
/**
...
/* Add the hour item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'cordillera' ) ) );
}
/**
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'cordillera' ) ) );
$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'cordillera' ) ) );
$day = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'cordillera' ) ) );
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'cordillera' ) ) );
$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'cordillera' ) ) );
/* Add the year item. */
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( $year ) . '">' . $year . '</a>';
/* Add the week item. */
if ( is_paged() )
...
if ( '%year%' == $tag )
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'cordillera' ) ) ) . '</a>';
/* If using the %monthnum% tag, add a link to the monthly archive. */
elseif ( '%monthnum%' == $tag )
$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'cordillera' ) ) ) . '</a>';
/* If using the %day% tag, add a link to the daily archive. */
elseif ( '%day%' == $tag )
$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'cordillera' ) ) ) . '</a>';
...
* @access public
*/
class Cordillera_bbPress_Breadcrumb_Trail extends Cordillera_Breadcrumb_Trail {
/**
