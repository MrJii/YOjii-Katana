 ...
 function cordillera_get_banner($type = 1){
    ...
    $banner_image_background = array(
	 ...
	 switch( $type ){
		 ...
		 $banner_text  = cordillera_options_array("banner_text",'');
  ...
  if( $detect->isMobile() || $detect->isTablet() ){
	  if( $poster ){
		 ...
		 $banner_text  = cordillera_options_array("video_banner_text",'');
   ...
   <li  <?php comment_class("comment"); ?> id="comment-<?php comment_ID() ;?>">
                                	<article class="comment-body">
                                        ...
                                        <div class="comment-box">
                                            <div class="comment-info"><?php printf(__('%s <span class="says">says:</span>','slugglera'), get_comment_author_link()) ;?> <a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ;?>">
<?php printf(__('%1$s at %2$s','slugglera'), get_comment_date(), get_comment_time()) ;?></a>  <?php edit_comment_link(__('(Edit)','slugglera'),'  ','') ;?></div>

 <?php if ($comment->comment_approved == '0') : ?>
         <em><?php _e('Your comment is awaiting moderation.','slugglera') ;?></em>
         <br />
      <?php endif; ?>
...
add_action( 'jihadcframework_sidebar','cordillera_options_panel_sidebar' );

/**
 * cordillera admin sidebar
 */
function cordillera_options_panel_sidebar() { ?>
	<div id="jihadcframework-sidebar">
...
<?php
 ...
 function cordillera_wp_title( $title, $sep ) {
	...
	if ( $paged >= 2 || $page >= 2 )
		$title = "$title $sep " . sprintf( __( ' Page %s ', 'slugglera' ), max( $paged, $page ) );
