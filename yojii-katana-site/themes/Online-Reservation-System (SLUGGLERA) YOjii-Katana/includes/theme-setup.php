...
function cordillera_setup(){
	...
	register_nav_menus( array('primary' => __( 'Primary Menu', 'slugglera' )));
	add_editor_style("editor-style.css");
	add_image_size( 'cordillera_portfolio_thumb', 480, 360 , true);
	add_image_size( 'cordillera_widget_blog', 720, 480 , true);
...
@media screen and (max-width: 640px) {
   ...
   function cordillera_admin_script(){
	   if( isset($_GET['page']) && $_GET['page'] == 'cordillera-options' )
	   {
           ...
           wp_enqueue_script( 'cordillera-admin', get_template_directory_uri().'/js/admin.js', array( 'jquery' ), $theme_info->get( 'Version' ), true );
		   }
...
add_action('jihadcframework_custom_scripts', 'cordillera_jihadcframework_custom_scripts');

function cordillera_jihadcframework_custom_scripts() { 

}

add_filter('jihadcframework_location','cordillera_jihadcframework_location_override');

function cordillera_jihadcframework_location_override() {
	return array('includes/admin-options.php');
}

function cordillera_optionscheck_options_menu_params( $menu ) {
	
	$menu['page_title'] = __( 'Slugglera Options', 'slugglera');
	$menu['menu_title'] = __( 'Slugglera Options', 'slugglera');
	$menu['menu_slug'] = 'cordillera-options';
	return $menu;
}
add_filter( 'jihadcframework_menu', 'cordillera_optionscheck_options_menu_params' );
...
if ( isset( $_POST['reset'] ) ) {
	 $output = array();
 $location = apply_filters( 'jihadcframework_location', array('admin-options.php') );

	        if ( $optionsfile = locate_template( $location ) ) {
	            $maybe_options = require_once $optionsfile;
	            if ( is_array( $maybe_options ) ) {
					$options = $maybe_options;
	            } else if ( function_exists( 'jihadcframework_options' ) ) {
					$options = jihadcframework_options();
				}
	        }
