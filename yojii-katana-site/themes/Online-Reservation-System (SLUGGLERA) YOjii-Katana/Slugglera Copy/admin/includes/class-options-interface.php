...
class Options_Framework_Interface {
   ...
   if(isset($list_array['section-widget-area-name']) && is_array($list_array['section-widget-area-name']) ){
    ...
    $list_item_str .= cordillera_widget_area_generator($list_item_array,false);
			   ...
			   <select name="widget_layout" id="widget_layout"><option value="1">'.__("choose column","slugglera").'</option>
			    	<option value="1">'.__("1 column","slugglera").'</option>
				    <option value="2">'.__("2 columns","slugglera").'</option>
					<option value="3">'.__("3 columns","slugglera").'</option>
					<option value="4">'.__("4 columns","slugglera").'</option>
				</select>
           <input type="text" id="list-item-val" size="20" name="list-item-val" value="" placeholder="'.__("Widget Area Name","slugglera").'" />';

				
		   
		//
                 $output  .= '<p style="display:block;clear: both;height:30px;"><a href="javascript:;" style="float:left;margin-right:20px;" class="button-primary" id="addItem">'.__("Add Item","slugglera").'</a> <span id="list-item-notice" style=" color:red;"></span></p></div>';
